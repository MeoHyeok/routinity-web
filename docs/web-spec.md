...
| `/` | `TodayPage` | 원탭 기록(기상→취침, 식사, 공부), 오늘 점수 링, 연속 달성일, 전날 대비 델타, 세션 캐리오버 배너, 오늘 타임라인(원시 로그 목록+개별 삭제) |
...
하단 탭바(오늘/분석/AI 코치)는 `TabLayout`으로 고정, 목표/설정은 스택 네비게이션(뒤로가기 `←`) 형태. 타임라인은 별도 화면이 아니라 오늘 화면 스크롤 하단에 통합돼있음(대표님 지시로 2026-08-20 변경 — 탭/네비게이션에 새 항목을 추가하는 대신 오늘 화면에 흡수).
