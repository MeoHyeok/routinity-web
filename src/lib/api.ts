 ...
 * has no 기상 at all, check yesterday for a still-open session and show that instead.
 */
export async function fetchTodayLogsWithCarryover(): Promise<{ logs: LogEntry[]; isCarryover: boolean }> {
  ...
  const isOpen = priorLogs.some((l) => l.type === 'wake') &&
    [...priorLogs].sort((a, b) => a.timestamp.localeCompare(b.timestamp)).at(-1)?.type !== 'sleep'
  if (!isOpen) return { logs: today, isCarryover: false }
